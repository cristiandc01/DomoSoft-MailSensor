...
void launchWeb(int webtype) {
  ...
  content = "<!DOCTYPE HTML>\r\n<html><head><meta name='viewport' content='width=device-width, initial-scale=1'></head><h3>Configurazione guidata WiFi DomoSoft</h3><h5>Dash Button</h5>";  if (bootMode) {
...
void setupAP(void) {
  ...
  st += "</ol>";
  delay(100);
  WiFi.softAP("DomoSoft Binary Sensor", "", 6);
...
void createWebServer(int webtype)
{
  if ( webtype == 1 ) {
    ...
    server.on("/HardReset", []() {
      ...
      Serial.print("ADV: Format system");
      for (int i = 0; i < 360; ++i) {
        EEPROM.write(i, 0);
      }
      EEPROM.commit();
